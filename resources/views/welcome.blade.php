...
<body class="bg-gradient-to-br from-neutral-100 to-white min-h-screen flex flex-col">
    ...
    <!-- Navigation/Header -->
    <nav class="flex justify-between items-center px-8 py-6 bg-neutral-700 border-b border-neutral-800 shadow-sm">
        <div class="flex items-center space-x-3">
            <img src="{{ asset('images/logo.png') }}" alt="Patriots Österreich Logo" class="h-12">
        ...
        <div>
            @auth
                <span class="mr-4 text-white">Hallo, {{ Auth::user()->first_name }}!</span>
                <a href="{{ route('dashboard') }}" class="px-4 py-2 bg-red-700 text-white rounded hover:bg-red-800 transition">Zum Dashboard</a>
            @else
                <a href="{{ route('login') }}" class="px-4 py-2 text-red-300 hover:underline">Login</a>
    ...
    <main class="flex-grow flex flex-col items-center justify-center text-center">
        <h1 class="text-4xl md:text-6xl font-extrabold text-neutral-900 mb-4 drop-shadow">
            Willkommen bei <span class="text-red-700">Patriots</span> <span class="text-neutral-700">Österreich</span>!
        </h1>
        <p class="text-lg text-neutral-700 mb-8 max-w-xl">
            Die Community für Patriots-Fans in Österreich.<br>
            Jetzt registrieren und Teil unserer rot-weißen Familie werden!
        </p>
        @guest
